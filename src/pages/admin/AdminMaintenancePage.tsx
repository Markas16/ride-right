...
import { useMaintenanceRecords, useCreateMaintenance, useAdvanceMaintenanceStatus } from "@/controllers/useMaintenanceController";
...
export default function AdminMaintenancePage() {
  ...
  const statusMutation = useAdvanceMaintenanceStatus();
              ...
              <form onSubmit={(e) => {
                ...
                <div className="space-y-2">
                  ...
                  <Select value={form.vehicle_id} onValueChange={(v) => setForm({ ...form, vehicle_id: v })}>
                    ...
                    <SelectContent>
                      {vehicles.map((v) => (
                        <SelectItem key={v.id} value={v.id}>{v.displayName} ({v.year})</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                  ...
                  <motion.div whileHover={{ x: 4 }} transition={{ duration: 0.2 }}>
                    <Card className="glass-card rounded-2xl border-border/30 hover:neon-border transition-all duration-300">
                      <CardContent className="p-4 flex items-center justify-between gap-4">
                        <div>
                          <p className="font-heading font-bold">{vehicle?.displayName ?? "Unknown"}</p>
                          <p className="text-sm text-muted-foreground">{r.serviceDate} · {r.description}</p>
                        </div>
                        <div className="flex items-center gap-3">
                          ...
                          {r.canStart() && (
                            <Button size="sm" variant="outline" className="rounded-xl gap-1" onClick={() => statusMutation.mutate({ id: r.id, currentStatus: r.status })}>
